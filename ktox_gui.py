...
def _resolve_vendor(mac):
    if not mac: return "—"
    mu = mac.upper()
    # Try 8-char prefix (AA:BB:CC)
    v = _OUI.get(mu[:8])
    if v: return v
    # Try without colons — some OUI tables use AABBCC format
    stripped = mu.replace(":", "")
...
class KTOxApp(ctk.CTk):
    ...
    def _fill_tab_wifi(self):
        ...
        # Interface selector — the most important thing in this tab
        self._sect(p, "INTERFACE")
        iface_frame = ctk.CTkFrame(p, fg_color="transparent", corner_radius=0)
        iface_frame.pack(fill="x", padx=6, pady=(0,4))

        ctk.CTkLabel(iface_frame, text="WiFi Interface",
            font=("Courier New", self._fs(7)),
            text_color=FG_ASH, anchor="w"
        ).pack(side="left", padx=(6,4))

        # Populate with detected wireless interfaces
        import ktox_wifi as _kw
        _ifaces = _kw._get_wireless_interfaces() or ["wlan0"]

        self._wifi_iface_var = ctk.StringVar(value=_ifaces[0])
        self._wifi_iface_menu = ctk.CTkOptionMenu(
            iface_frame,
            variable=self._wifi_iface_var,
            values=_ifaces,
            font=("Courier New", self._fs(8)),
            fg_color=BG_INPUT,
            button_color=RED_RUST,
            button_hover_color=RED_BLOOD,
            text_color=FG_WHITE,
            dropdown_fg_color=BG_DARK,
            dropdown_text_color=FG_WHITE,
            dropdown_hover_color=RED_RUST,
            width=120, height=26,
            corner_radius=2,
            command=lambda v: self._log(f"WiFi interface → {v}", "dim")
        )
        self._wifi_iface_menu.pack(side="left", padx=4)

        # Refresh button — re-scans for adapters (e.g. plugged in USB dongle)
        ctk.CTkButton(iface_frame, text="⟳",
            font=("Courier New", self._fs(9)), width=28, height=26,
            fg_color="transparent", border_color=RED_RUST, border_width=1,
            text_color=FG_ASH, hover_color=BG_INPUT, corner_radius=2,
            command=self._wifi_refresh_ifaces
    ...
    def _build_right(self, parent):
        ...
        # Fixed minimum widths — stretch=False so horizontal scroll works
        for col, w in zip(cols, [30, 130, 150, 120, 150, 44]):
            self._tree.heading(col, text=col,
                command=lambda c=col: self._tree.heading(c, text=c))
            self._tree.column(col, width=w, minwidth=w, stretch=False, anchor="w")
        # Vertical scrollbar
        vsb = ttk.Scrollbar(tf, orient="vertical",   command=self._tree.yview)
        # Horizontal scrollbar — lets you swipe left/right on small screens
        xsb = ttk.Scrollbar(tf, orient="horizontal", command=self._tree.xview)
        self._tree.configure(yscrollcommand=vsb.set, xscrollcommand=xsb.set)
        self._tree.grid(row=0, column=0, sticky="nsew")
        vsb.grid(row=0, column=1, sticky="ns")
        xsb.grid(row=1, column=0, sticky="ew")
        # Bind mousewheel for horizontal scroll (Shift+scroll or trackpad)
        self._tree.bind("<Shift-MouseWheel>",
            lambda e: self._tree.xview_scroll(int(-1*(e.delta/120)), "units"))
        ...
        self._tree.bind("<Button-5>",
            lambda e: self._tree.yview_scroll(1, "units"))
        self._tree.bind("<Shift-Button-4>",
            lambda e: self._tree.xview_scroll(-1, "units"))
        self._tree.bind("<Shift-Button-5>",
            lambda e: self._tree.xview_scroll(1, "units"))
    ...
    def _resize_tree(self):
        """Ensure columns are at least their minimum widths.
        Does NOT shrink columns below minimums — horizontal scroll handles overflow."""
        try:
            tw = self._tree_frame.winfo_width() - 24  # subtract scrollbar width
            if tw < 80: return
            # Minimum widths per column
            mins = {"#": 30, "IP": 130, "MAC": 150, "VENDOR": 120, "HOST": 150, "ROLE": 44}
            total_min = sum(mins.values())  # 624px minimum
            if tw >= total_min:
                # Screen is wide enough — distribute extra space to IP, HOST, VENDOR
                extra = tw - total_min
                add = {"IP": 0.25, "MAC": 0.1, "VENDOR": 0.25, "HOST": 0.35, "#": 0, "ROLE": 0.05}
                for col, w in mins.items():
                    self._tree.column(col, width=int(w + extra * add.get(col, 0)))
            else:
                # Screen too narrow — use minimums, horizontal scroll handles the rest
                for col, w in mins.items():
    ...
    def _fill_tree(self):
        ...
        for i, h in enumerate(hosts_list):
            ip     = h[0]
            mac    = h[1]
            vendor = h[2] if len(h) > 2 else _resolve_vendor(mac)
            host   = h[3] if len(h) > 3 else "…"
            gw     = ip == default_gateway_ip
            self._tree.insert("", "end",
                values=(i, ip, mac, vendor, host, "GW" if gw else "OK"),
    ...
    def _stop(self):
        # Signal all stop flags — both ktox_gui and any loaded engine modules
        stop_flag.set()
        try:
            import ktox_wifi;     ktox_wifi.stop_flag.set()
        except: pass
        try:
            import ktox_mitm;     ktox_mitm.stop_flag.set()
        except: pass
        try:
            import ktox_advanced; ktox_advanced.stop_flag.set()
        except: pass
        try:
            import ktox_netattack; ktox_netattack.stop_flag.set()
        except: pass

        if not self._attacking:
            self._log("Stopped all running modules.", "dim")
            self._setstatus("● READY", GREEN)
            return
    ...
    # ── WiFi handlers ─────────────────────────────────────────────────────────
    def _wifi_refresh_ifaces(self):
        """Re-scan for wireless interfaces and update the dropdown."""
        def _run():
            try:
                import ktox_wifi
                ifaces = ktox_wifi._get_wireless_interfaces()
                if not ifaces:
                    self._log("No wireless interfaces found.", "orange")
                    return
                current = self._wifi_iface_var.get()
                self.after(0, lambda: self._wifi_iface_menu.configure(values=ifaces))
                if current not in ifaces:
                    self.after(0, lambda: self._wifi_iface_var.set(ifaces[0]))
                    self._log(f"Interface reset -> {ifaces[0]}", "dim")
                else:
                    self._log(f"Interfaces: {', '.join(ifaces)}", "dim")
            except Exception as ex:
                self._log(f"Refresh error: {ex}", "red")
        threading.Thread(target=_run, daemon=True).start()

    def _wifi_get_iface(self):
        """Get the currently selected WiFi interface from the dropdown."""
        try:
            return self._wifi_iface_var.get()
        except:
            import ktox_wifi
            ifaces = ktox_wifi._get_wireless_interfaces()
    ...
    def _wifi_monitor_on(self):
        ...
        self._log(f"Enabling monitor mode on {iface}...", "yellow")
        def _run():
            try:
                import ktox_wifi
                mon    = ktox_wifi.MonitorMode(iface)
                mon_if = mon.enable()
                self._log(f"Monitor mode active: {mon_if}", "green")
                self._log("Press Disable Monitor when done.", "dim")
                # Update dropdown to the monitor interface name
                try:
                    ifaces = ktox_wifi._get_wireless_interfaces()
                    if ifaces:
                        self.after(0, lambda: self._wifi_iface_menu.configure(values=ifaces))
                    self.after(0, lambda: self._wifi_iface_var.set(mon_if))
                except: pass
            except Exception as ex:
    ...
    def _wifi_monitor_off(self):
        ...
        def _run():
            try:
                ...
                # Happy path — use the class registry
                if ktox_wifi.MonitorMode._active:
                    ktox_wifi.MonitorMode.disable_all()
                    self._log("Monitor mode disabled. NetworkManager restarted.", "green")
                    return

                # No registered instance — scan for any interface still in monitor mode
                self._log("No tracked instance — scanning for monitor interfaces...", "dim")
                result = ktox_wifi._run(["iw", "dev"])
                current = None
                restored = 0
                for line in result.stdout.splitlines():
                    ...
                    if line.startswith("type monitor") and current:
                        self._log(f"Restoring {current} to managed...", "dim")
                        ktox_wifi._run(["ip", "link", "set", current, "down"])
                        ktox_wifi._run(["iw", current, "set", "type", "managed"])
                        ktox_wifi._run(["ip", "link", "set", current, "up"])
                        restored += 1
                        current = None  # reset so we don't double-process
                if restored:
                    ktox_wifi._run(["systemctl", "start", "NetworkManager"])
                    self._log(
                        f"Restored {restored} interface(s) + restarted NetworkManager.",
                        "green"
                    )
                else:
                    self._log("No interfaces in monitor mode found.", "dim")

            except Exception as ex:
                self._log(f"Disable monitor error: {ex}", "red")
        threading.Thread(target=_run, daemon=True).start()
    ...
    def _on_close(self):
        ...
        if self._attacking and self._attack_targets:
            for _ in range(5):
                ...
                time.sleep(0.1)
        # Restore any monitor mode interfaces — critical to avoid broken wlan0
        try:
            import ktox_wifi
            if ktox_wifi.MonitorMode._active:
                self._log("Restoring monitor mode interfaces on exit...", "orange")
                ktox_wifi.MonitorMode.disable_all()
        except: pass
        self.destroy()
