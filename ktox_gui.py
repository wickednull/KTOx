    ...
    def _wifi_monitor_on(self):
        ...
        def _run():
            try:
                ...
                if not ifaces:
                    self._log("No wireless interfaces found.", "red")
                    return
                iface = ifaces[0]
                self._log(f"Using interface: {iface}", "dim")
                mon    = ktox_wifi.MonitorMode(iface)
                mon_if = mon.enable()
                # MonitorMode._active registry is set inside enable()
                # so disable_all() can always find and restore it
                self._log(f"Monitor mode active: {mon_if}", "green")
                self._log("Press Disable Monitor when done — restores NetworkManager.", "dim")
            except Exception as ex:
