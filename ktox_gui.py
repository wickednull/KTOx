    ...
    def _wifi_monitor_off(self):
        ...
        def _run():
            try:
                ...
                # Use the class registry — finds whichever instance called enable()
                active = ktox_wifi.MonitorMode._active
                if not active:
                    # No registered instance — try to restore all wireless
                    # interfaces from monitor mode using iw directly
                    self._log("No tracked monitor instance — scanning for monitor interfaces...", "dim")
                    ...
                    if restored:
                        ...
                        self._log(f"Restored {restored} interface(s) + restarted NetworkManager.", "green")
                    else:
                        self._log("No interfaces in monitor mode found.", "dim")
                    return

                # Disable all tracked instances
