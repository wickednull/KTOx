...
def print_host_table(highlight_ips=None):
    ...
    table.add_column("#",       style=f"bold {C_BLOOD}", width=3,  no_wrap=True)
    table.add_column("IP",      style=C_WHITE,           width=13, no_wrap=True)
    table.add_column("MAC",     style=C_STEEL,           width=17, no_wrap=True)
    table.add_column("VENDOR",  style=C_DIM,             width=14, no_wrap=True)
    table.add_column("HOST",    style=C_DIM,             width=14, no_wrap=True)
    table.add_column("ST",      style=C_GOOD,            width=3,  no_wrap=True)

    for i, host in enumerate(hosts_list):
        ip       = host[0]
        mac      = host[1]
        vendor   = resolve_vendor(mac)[:13]
        hostname = resolve_hostname(ip)[:13]
