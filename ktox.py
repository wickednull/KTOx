...
import os, sys, time, threading, logging, math, socket, datetime, json
...
def ask_packets(default=6):
    val = Prompt.ask(
        f"  [{C_STEEL}]Packets per minute [{C_DIM}]default={default}[/{C_DIM}][/{C_STEEL}]",
        default=str(default)
    )
    try:
        return int(val)
...
def get_gateway_ip():
    try:
        pkt, timeout = sr1(IP(dst="8.8.8.8", ttl=0)/ICMP()/"PROBE", verbose=False, timeout=4)
        if not timeout and pkt:
            return pkt.src
    except:
        pass
...
def resolve_vendor(mac):
    try:
        ...
        req = Request(f"http://macvendors.co/api/vendorname/{mac}",
                      headers={"User-Agent": "API Browser"})
        res = urlopen(req, timeout=2)
        return res.read().decode("utf-8")[:22]
    except:
        return "Unknown"
...
def print_host_table(highlight_ips=None):
    ...
    table.add_column("#",          style=f"bold {C_BLOOD}", width=4)
    table.add_column("IP ADDRESS", style=C_WHITE,           width=16)
    table.add_column("MAC",        style=C_STEEL,           width=18)
    table.add_column("VENDOR",     style=C_DIM,             width=24)
    table.add_column("HOSTNAME",   style=C_DIM,             width=28)
    table.add_column("STATUS",     style=C_GOOD,            width=10)
    ...
    for i, host in enumerate(hosts_list):
        ...
        status   = f"[{C_YELLOW}]GATEWAY[/{C_YELLOW}]" if is_gw else f"[{C_GOOD}]ONLINE[/{C_GOOD}]"
        table.add_row(
    ...
    while not stop_flag.is_set():
        for ip, mac in targets:
            if mac:
                ...
                sent += 1
        time.sleep(interval)
    log("SPOOF_LOOP_END", packets_sent=sent)
...
def run_attack(targets, label_str, ppm=None):
    stop_flag.clear()
    t = threading.Thread(target=_spoof_loop, args=(targets, ppm), daemon=True)
    t.start()
    ip_list = ", ".join(ip for ip, _ in targets)
    rate    = f"{ppm} pkts/min" if ppm else "6 pkts/min (default)"
    log("ATTACK_START", mode=label_str, targets=ip_list, rate=rate)
...
def mode_kick_one():
    ...
    ppm = ask_packets()
    run_attack(targets, "SINGLE TARGET", ppm)
...
def mode_kick_some():
    ...
    ppm = ask_packets()
    run_attack(targets, f"{len(targets)} TARGETS", ppm)
...
def mode_kick_all():
    ...
    ppm = ask_packets()
    run_attack(targets, "ALL HOSTS", ppm)
...
def print_status_bar():
    ...
    console.print(
        f"  [{C_RUST}]▐[/{C_RUST}] "
        f"[{C_STEEL}]{default_interface}  {default_iface_mac}[/{C_STEEL}]"
        f"  [{C_DIM}]|[/{C_DIM}]  [{C_EMBER}]GW {default_gateway_ip}[/{C_EMBER}]"
        f"  [{C_DIM}]|[/{C_DIM}]  [{C_GOOD}]{len(hosts_list)} host(s)[/{C_GOOD}]"
        f"  [{C_DIM}]|[/{C_DIM}]  [{C_DIM}]log → {LOG_FILE}[/{C_DIM}]"
        f"  [{C_RUST}]▌[/{C_RUST}]"
    )
    console.print(Rule(style=C_RUST))
...
def main():
    ...
    with Progress(
        SpinnerColumn(spinner_name="bouncingBall", style=C_BLOOD),
        TextColumn(f"[{C_STEEL}]Initialising network stack...[/{C_STEEL}]"),
        transient=True,
    ) as prog:
        prog.add_task("init")
